...
Локации в игре разбиты на несколько файлов: LAND для геометрии (карта высот, лоды, данные для работы алгоритма оптимизации рендеринга карты высот ROAM), ETL текстуры и индексы для детализации текстур, LIGHT - изображения в формате JPEG (intel jpeg library), цветовая карта, SCBN - объекты на локации и другие.  
##### Другое 
...
|  **2**  | [light_tool](/tools/gen/land_tool)  | Генерация карты неровностей (коэффициент d2) из карты высот |
|  **3**  | [light_tool](/tools/квадродерево)  | Парсер .ai файлов (квадродерево ноды для разбиения пространства) |
