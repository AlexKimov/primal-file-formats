...
В игре Осада добавлен упрощенный формат для моделей с тем же расширением.
##### Форматы для моделей и текстур  
Для текстур используются форматы DDS и TGA, которые хранятся в соотвествующих файлах, а также в файлах ETL хранятся текстуры в DDS формате. Модели в игре лежат в файлах MESH. Анимации для моделей в файлах ANM. Локации в игре разбиты на несколько файлов: LAND для геометрии, ETL для текстур и другие.  
##### Другое 
Текстовые файлы, которые содержат различные параметры объектов и настройки, зашифрованы (ниже).

### Вопросы/Ответы

1. Как достать модели из игры Глаз Дракона? Как вытащить модели из игры Осада? Как открыть файл .msh?  
   Использовать плагин для Blender в разработке ([ссылка](#blender))  Поддерживаются старые версии программы, данные для анимации на данный момент не считываются.  
   Использовать плагин для Noesis ([ссылка](#noesis)). Считываются все данные, анимации не отображаются.  
2. Как открыть файлы .city, .dsc, .dat, .fir и другие?  
   ИСпользовать скрипт для 010Editor ([ссылка](#010editor)).  

### Задачи:

##### Глаз дракона (2002) / Осада (2004)
- [ ] Анализ форматов  
...
|  **2**  | MESH | [MESH.bt](templates/010editor/MESH.bt)  | (некоторые) трехмерные модели | 
|  **2**  | MESH | [MESH_besieger.bt](templates/010editor/MESH_besieger.bt)  | (большая часть) трехмерные модели |
|  **3**  | LAND | [LAND.bt](templates/010editor/LAND.bt)  | Поверхность уровня (карта высот) | 
...
| № | Скрипт | Описание  |
| :-- | :------- | :-------  | 
|  **1**  | [idragon_unpack_res.bms](scripts/idragon_unpack_res.bms)  | Скрипт для распаковки файлов игры Глаз Дракона (2002) и Осада (2004) |   
