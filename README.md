...
Для текстур используются форматы DDS и TGA, которые хранятся отдельно в соответствующих файлах, а также в файлах ETL хранятся текстуры в DDS формате и в LIGHT - JPEG текстуры. Модели в игре лежат в файлах MSH. Анимации для моделей в файлах ANM. 

Локации в игре разбиты на несколько файлов: LAND для геометрии (карта высот, лоды, данные для работы алгоритма оптимизации рендеринга карты высот ROAM), ETL текстуры и индексы для детализации текстур, LIGHT - изображения в формате JPEG (intel jpeg library), цветовая карта, SCBN - объекты на локации, .ai ноды квадрадерева для работы ии и другие.  
##### Другое 
...
0. Текущий статус? Где что лежит и что с этим делать?   
   На данный момент исследованы форматы моделей, анимации, локаций и файлы ресурсов. Их можно загружать и просматривать, есть описание форматов для создание своих программ.
   В папке **plugins** плагины для работы с файлами игр, в папке **scripts** инструменты для работы с файлом ресурсов .res и расшифровки текстовых файлов в игреб в папке **tools** - программы для работы с файлами.
...
- [x] Плагин BLENDER для просмотра .land файлов
...
|  **4**  | ETL | [ETL.bt](templates/010editor/ETLS.bt)  | Тайлы детализации текстур поверхностей уровня |  
|  **4**  | LIGHT | [ETL.bt](templates/010editor/LIGHT.bt)  | Текстуры с цветом |  
...
| № | Скрипт | Описание  |
| :-- | :------- | :-------  | 
|  **1**  | [light_tool](/tools/io/light_tool)  | Распаковка текстур из файла .light |
|  **2**  | [land_too](/tools/gen/land_tool)  | Генерация карты неровностей (коэффициент d2) из карты высот |
|  **3**  | [ai_tool](/tools/ai_tool)  | Парсер .ai файлов (квадродерево ноды для разбиения пространства) |
...
|  **2**  | [io_mesh_land](plugins/blender/io_mesh_land/__init__.py)  | Просмотр карты высот из игр Глаз Дракона (2002) и Осада (2004) |  |
